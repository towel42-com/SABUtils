...
namespace NTowel42Utils
{
    namespace NFileUtils
    {
        ...
        public:
            ...
            SFileInfo fillFileInfo( int index ) const;
        ...
        SFileInfo QZipPrivate::fillFileInfo( int index ) const
        {
            SFileInfo fileInfo;
            ...
            switch ( hostOS )
            {
                case HostUnix:
                    ...
                    switch ( mode & UnixFileAttributes::TypeMask )
                    {
                        case UnixFileAttributes::SymLink:
                            fileInfo.fIsSymLink = true;
                            break;
                        case UnixFileAttributes::Dir:
                            fileInfo.fIsDir = true;
                            break;
                        case UnixFileAttributes::File:
                        default:   // ### just for the case; should we warn?
                            fileInfo.fIsFile = true;
                            break;
                    }
                    fileInfo.fPermissions = modeToPermissions( mode );
                    break;
                case HostFAT:
                case HostNTFS:
                case HostHPFS:
                case HostVFAT:
                    ...
                    fileInfo.fPermissions |= QFile::ReadOwner | QFile::ReadUser | QFile::ReadGroup | QFile::ReadOther;
                    if ( ( mode & WindowsFileAttributes::ReadOnly ) == 0 )
                        fileInfo.fPermissions |= QFile::WriteOwner | QFile::WriteUser | QFile::WriteGroup | QFile::WriteOther;
                    if ( fileInfo.fIsDir )
                        fileInfo.fPermissions |= QFile::ExeOwner | QFile::ExeUser | QFile::ExeGroup | QFile::ExeOther;
                    break;
                default:
            ...
            fileInfo.fFilePath = inUtf8 ? QString::fromUtf8( header.file_name ) : QString::fromLocal8Bit( header.file_name );
            fileInfo.fCRC = readUInt( header.h.crc_32 );
            fileInfo.fSize = readUInt( header.h.uncompressed_size );
            fileInfo.fLastModified = readMSDosDate( header.h.last_mod_file );

            // fix the file path, if broken (convert separators, eat leading and trailing ones)
            fileInfo.fFilePath = QDir::fromNativeSeparators( fileInfo.fFilePath );
            QStringView filePathRef( fileInfo.fFilePath );
            while ( filePathRef.startsWith( u'.' ) || filePathRef.startsWith( u'/' ) )
                filePathRef = filePathRef.mid( 1 );
            while ( filePathRef.endsWith( u'/' ) )
                filePathRef.chop( 1 );

            fileInfo.fFilePath = filePathRef.toString();
            return fileInfo;
        }

        class CZipReaderPrivate : public QZipPrivate
        {
        public:
            CZipReaderPrivate( QIODevice *device, bool ownDev ) :
                QZipPrivate( device, ownDev ),
                status( EStatus::eNoError )
            {
            }
        ...
        class CZipWriterPrivate : public QZipPrivate
        {
        public:
            CZipWriterPrivate( QIODevice *device, bool ownDev ) :
                QZipPrivate( device, ownDev ),
                status( EStatus::eNoError ),
                permissions( QFile::ReadOwner | QFile::WriteOwner ),
                compressionPolicy( ECompressionPolicy::eAutoCompress )
            {
            }

            EStatus status;
            QFile::Permissions permissions;
            ECompressionPolicy compressionPolicy;
        ...
        void CZipReaderPrivate::scanFiles()
        {
            if ( !dirtyFileTree )
                return;
        ...
        void CZipWriterPrivate::addEntry( EntryType type, const QString &fileName, const QByteArray &contents /*, QFile::Permissions permissions, QZip::Method m*/ )
        {
#ifndef NDEBUG
            static const char * const entryTypes[] = { "directory", "file     ", "symlink  " };
            ...
            if ( compressionPolicy == ECompressionPolicy::eAutoCompress )
            {
                if ( contents.size() < 64 )
                    compression = ECompressionPolicy::eNeverCompress;
                else
                    compression = ECompressionPolicy::eAlwaysCompress;
            }
            ...
            if ( compression == ECompressionPolicy::eAlwaysCompress )
            {
...
*/
        CZipReader::CZipReader( const QFileInfo &fi, QIODevice::OpenMode mode ) :
            CZipReader( fi.absoluteFilePath(), mode )
        {
        }

        CZipReader::CZipReader( const QString &archive, QIODevice::OpenMode mode )
        {
            ...
            else
            {
                if ( error == QFile::ReadError )
                    ...
                    status = EStatus::eFilePermissionsError;
                else
                    status = EStatus::eFileError;
            }

            fImpl = std::make_unique< CZipReaderPrivate >( f.get(), /*ownDevice=*/true );
            Q_UNUSED( f.release() );
            fImpl->status = status;
        }
 ...
 */
        CZipReader::CZipReader( QIODevice *device ) :
            fImpl( new CZipReaderPrivate( device, /*ownDevice=*/false ) )
        {
            Q_ASSERT( device );
        }
...
*/
        QIODevice *CZipReader::device() const
        {
            return fImpl->device;
        }
...
*/
        bool CZipReader::isReadable() const
        {
            return fImpl->device->isReadable();
        }

        /*!
    Returns \c true if the file exists; otherwise returns \c false.
*/
        bool CZipReader::exists() const
        {
            QFile *f = qobject_cast< QFile * >( fImpl->device );
            if ( f == nullptr )
                return true;
            return f->exists();
        }
...
*/
        QList< SFileInfo > CZipReader::fileInfoList() const
        {
            fImpl->scanFiles();
            QList< SFileInfo > files;
            const int numFileHeaders = fImpl->fileHeaders.size();
            files.reserve( numFileHeaders );
            for ( int i = 0; i < numFileHeaders; ++i )
                files.append( fImpl->fillFileInfo( i ) );
            return files;
        }
...
*/
        int CZipReader::count() const
        {
            fImpl->scanFiles();
            return fImpl->fileHeaders.size();
        }
    ...
    \sa fileInfoList()
*/
        std::optional< SFileInfo > CZipReader::entryInfoAt( int index ) const
        {
            fImpl->scanFiles();
            if ( index >= 0 && index < fImpl->fileHeaders.size() )
                return fImpl->fillFileInfo( index );
            return {};
        }
...
*/
        std::optional< QByteArray > CZipReader::fileData( const QString &fileName ) const
        {
            fImpl->scanFiles();
            int i;
            for ( i = 0; i < fImpl->fileHeaders.size(); ++i )
            {
                if ( QString::fromLocal8Bit( fImpl->fileHeaders.at( i ).file_name ) == fileName )
                    break;
            }
            if ( i == fImpl->fileHeaders.size() )
                return {};

            FileHeader header = fImpl->fileHeaders.at( i );
            ...
            fImpl->device->seek( start );
            LocalFileHeader lh;
            fImpl->device->read( (char *)&lh, sizeof( LocalFileHeader ) );
            uint skip = readUShort( lh.file_name_length ) + readUShort( lh.extra_field_length );
            fImpl->device->seek( fImpl->device->pos() + skip );
            ...
            //qDebug("file at %lld", fImpl->device->pos());
            QByteArray compressed = fImpl->device->read( compressed_size );
            ...
            else if ( compression_method == CompressionMethodDeflated )
            {
                ...
                {
                    baunzip.resize( len );
                    res = inflate( (uchar *)baunzip.data(), &len, (const uchar *)compressed.constData(), compressed_size );
...
*/
        bool CZipReader::extractAll( const QString &destinationDir ) const
        {
            QDir baseDir( destinationDir );

            // create directories first
            const QList< SFileInfo > allFiles = fileInfoList();
            bool foundDirs = false;
            bool hasDirs = false;
            for ( const SFileInfo &fi : allFiles )
            {
                const QString absPath = destinationDir + QDir::separator() + fi.fFilePath;
                if ( fi.fIsDir )
                {
                    foundDirs = true;
                    if ( !baseDir.mkpath( fi.fFilePath ) )
                        return false;
                    if ( !QFile::setPermissions( absPath, fi.fPermissions ) )
                        return false;
                }
                else if ( !hasDirs && fi.fFilePath.contains( u"/" ) )
                {
            ...
            if ( hasDirs && !foundDirs )
            {
                for ( const SFileInfo &fi : allFiles )
                {
                    const auto dirPath = fi.fFilePath.left( fi.fFilePath.lastIndexOf( u"/" ) );
            ...
            for ( const SFileInfo &fi : allFiles )
            {
                ...
                if ( fi.fIsSymLink )
                {
                    auto data = fileData( fi.fFilePath );
                    if ( !data.has_value() )
                        return false;

                    QString destination = QFile::decodeName( data.value() );
            ...
            for ( const SFileInfo &fi : allFiles )
            {
                ...
                if ( fi.fIsFile )
                {
                    ...
                    if ( !data.has_value() )
                        return false;
                    f.write( data.value() );
                    f.setPermissions( fi.fPermissions );
                    f.close();
                }
            }
...
*/
        EStatus CZipReader::status() const
        {
            return fImpl->status;
        }

        QString CZipReader::statusText() const
        {
...
*/
    ...
    \sa isValid()
*/
        CZipWriter::CZipWriter( const QFileInfo &fi, QIODevice::OpenMode mode ) :
            CZipWriter( fi.absoluteFilePath(), mode )
        {
        }

        CZipWriter::CZipWriter( const QString &fileName, QIODevice::OpenMode mode )
        {
            ...
            else
            {
                if ( f->error() == QFile::WriteError )
                    status = EStatus::eFileWriteError;
            ...
            fImpl = std::make_unique< CZipWriterPrivate >( f.get(), /*ownDevice=*/true );
            Q_UNUSED( f.release() );
            fImpl->status = status;
        }
 ...
 */
        CZipWriter::CZipWriter( QIODevice *device ) :
            fImpl( new CZipWriterPrivate( device, /*ownDevice=*/false ) )
        {
            Q_ASSERT( device );
        }
...
*/
        QIODevice *CZipWriter::device() const
        {
            return fImpl->device;
        }
...
*/
        bool CZipWriter::isWritable() const
        {
            return fImpl->device->isWritable();
        }

        /*!
    Returns \c true if the file exists; otherwise returns \c false.
*/
        bool CZipWriter::exists() const
        {
            QFile *f = qobject_cast< QFile * >( fImpl->device );
            if ( f == nullptr )
                return true;
...
*/
    ...
    or NoError if no error occurred.
*/
        EStatus CZipWriter::status() const
        {
            return fImpl->status;
        }

        QString CZipWriter::statusText() const
        {
            return toText( status() );
        }
...
*/
    ...
    \sa addFile()
*/
        void CZipWriter::setCompressionPolicy( ECompressionPolicy policy )
        {
            fImpl->compressionPolicy = policy;
        }
    ...
    \sa addFile()
*/
        ECompressionPolicy CZipWriter::compressionPolicy() const
        {
            return fImpl->compressionPolicy;
        }
    ...
    \sa addFile()
*/
        QFile::Permissions CZipWriter::setCreationPermissions( QFile::Permissions permissions )
        {
            auto curr = fImpl->permissions;
            fImpl->permissions = permissions;
            return curr;
        }
    ...
    \sa addFile()
*/
        QFile::Permissions CZipWriter::creationPermissions() const
        {
            return fImpl->permissions;
        }

        void CZipWriter::addItem( const SFileInfo &fileInfo, const std::optional< QByteArray > &data )
        {
            auto currPerms = setCreationPermissions( fileInfo.fPermissions );
            if ( fileInfo.fIsFile )
            {
                if ( !data.has_value() )
                    fImpl->status = EStatus::eFileWriteError;
                else
                    addFile( fileInfo.fFilePath, data.value() );
            }
            else if ( fileInfo.fIsDir )
            {
                addDirectory( fileInfo.fFilePath );
            }
            else if ( fileInfo.fIsSymLink )
            {
                if ( !data.has_value() )
                    fImpl->status = EStatus::eFileWriteError;
                else
    ...
    \sa setCompressionPolicy()
*/
        void CZipWriter::addFile( const QString &fileName, const QByteArray &data )
        {
            fImpl->addEntry( CZipWriterPrivate::File, QDir::fromNativeSeparators( fileName ), data );
        }
    ...
    includes the full path in the archive.
*/
        void CZipWriter::addFile( const QString &fileName, QIODevice *device )
        {
            Q_ASSERT( device );
            ...
            if ( ( mode & QIODevice::ReadOnly ) == 0 )
            {
                ...
                {
                    fImpl->status = EStatus::eFileOpenError;
                    return;
                }
            }
            fImpl->addEntry( CZipWriterPrivate::File, QDir::fromNativeSeparators( fileName ), device->readAll() );
            if ( opened )
                device->close();
        }
    ...
    the \a permissions;
*/
        void CZipWriter::addDirectory( const QString &dirName )
        {
            QString name( QDir::fromNativeSeparators( dirName ) );
            // separator is mandatory
            if ( !name.endsWith( u'/' ) )
                name.append( u'/' );
            fImpl->addEntry( CZipWriterPrivate::Directory, name, QByteArray() );
        }
...
*/
        void CZipWriter::addSymLink( const QString &fileName, const QString &destination )
        {
            fImpl->addEntry( CZipWriterPrivate::Symlink, QDir::fromNativeSeparators( fileName ), QFile::encodeName( destination ) );
        }

        /*!
   Closes the zip file.
*/
        void CZipWriter::close()
        {
            if ( !( fImpl->device->openMode() & QIODevice::WriteOnly ) )
            {
                fImpl->device->close();
                return;
            }

            //qDebug("QZip::close writing directory, %d entries", fImpl->fileHeaders.size());
            fImpl->device->seek( fImpl->start_of_directory );
            // write new directory
            for ( int i = 0; i < fImpl->fileHeaders.size(); ++i )
            {
                const FileHeader &header = fImpl->fileHeaders.at( i );
                fImpl->device->write( (const char *)&header.h, sizeof( CentralFileHeader ) );
                fImpl->device->write( header.file_name );
                fImpl->device->write( header.extra_field );
                fImpl->device->write( header.file_comment );
            }
            int dir_size = fImpl->device->pos() - fImpl->start_of_directory;
            ...
            //uchar this_disk[2];
            //uchar start_of_directory_disk[2];
            writeUShort( eod.num_dir_entries_this_disk, fImpl->fileHeaders.size() );
            writeUShort( eod.num_dir_entries, fImpl->fileHeaders.size() );
            writeUInt( eod.directory_size, dir_size );
            writeUInt( eod.dir_start_offset, fImpl->start_of_directory );
            writeUShort( eod.comment_length, fImpl->comment.size() );

            fImpl->device->write( (const char *)&eod, sizeof( EndOfDirectory ) );
            fImpl->device->write( fImpl->comment );
            fImpl->device->close();
        }
        ...
        QString toText( EStatus status )
        {
            switch ( status )
            {
                case EStatus::eNoError:
                    return "No Error";
                case EStatus::eFileReadError:
                    return "Error reading file";
                case EStatus::eFileWriteError:
                    return "Error writing file";
                case EStatus::eFileOpenError:
                    return "Could not open file";
                case EStatus::eFilePermissionsError:
                    return "Error with file permissions";
                case EStatus::eFileError:
                default:
                    return "Unknown file error";
            }
        }

        SFileInfo::SFileInfo() noexcept :
            fIsDir( false ),
            fIsFile( false ),
            fIsSymLink( false ),
            fCRC( 0 ),
